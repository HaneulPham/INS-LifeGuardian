...
The project recognizes `analytics`, `write test cases`, `review test cases`, `Update test cases to Second Brain`, `write a bug`, and the Playwright API command chain as explicit workflow intents. Their strict boundaries live in `references/workflow-commands.md`; API implementation is isolated in `ins-lifeguardian-playwright-api-automation` and `automation/api/`. Web automation remains separate and is not loaded or generated by the API skill.
...
7. Use `write a bug` only for defects. For API automation, use `write`, `review`, `run`, `debug`, `fix`, and `update mapping` commands routed to the API-only Playwright project.
...
cd automation/api && npm install && npm run validate
```
